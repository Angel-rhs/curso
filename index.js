...
function getPost(){
    ...
    firebase.database().ref('comentarios').once('value', function(snapshot) {       
        ...
        for(var i=0;i<mostrar.length;i++){
            if(mostrar[i].tipoPost==="maPost"){
                data+="<div class='card' id='"+mostrar[i].key+"'><div class='card-body'><h5 class='card-title'>"+mostrar[i].userEmail+"<p class='card-text'>"+mostrar[i].comentario+"</p></h5><button type='submit'  class='btn btn-secondary' onclick=\"newPost(event,\'"+mostrar[i].key+"\')\"><i class='far fa-comments'></i> Comentarios</button> <button type='submit'  class='btn btn-primary' onclick=\"updateLike(\'"+mostrar[i].key+"\')\"><i class='far fa-thumbs-up'></i> Likes</button> <button type=''  class='btn btn-secondary ml-auto'>"+mostrar[i].likes.length+"</button><div class='container' id='"+mostrar[i].key+"'></div></div></div><br>";
            }else{
                data+="<div class='card' id='"+mostrar[i].key+"'><div class='card-body'><h5 class='card-title'>"+mostrar[i].userEmail+"<p class='card-text'>"+mostrar[i].comentario+"</p></h5><button type='submit'  class='btn btn-secondary' onclick=\"newPost(event,\'"+mostrar[i].key+"\')\"><i class='far fa-comments'></i> Comentarios</button> <button type='submit'  class='btn btn-primary' onclick=\"updateLike(\'"+mostrar[i].key+"\')\"><i class='far fa-thumbs-up'></i> Likes</button> <button type=''  class='btn btn-secondary ml-auto'>"+mostrar[i].likes.length+"</button><div class='container' id='"+mostrar[i].key+"'></div><br>";

                for(var j=0;j<mostrar[i].subPost.length;j++){
                    data+="<br><div class='card bg-light mb-3'><div class='card-header'>"+mostrar[i].subPost[j].idUser+"</div><div class='card-body'><p class='card-text'>"+mostrar[i].subPost[j].postComented+"</p></div><br>hola";
                }
                if(mostrar.length==i){
                    data+="</div></div></div><br>";
                }
